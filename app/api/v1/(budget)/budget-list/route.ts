  ...
  try {
    ...
    const budgetList = await db
      ...
      .select(({ fn }) => [
        ...
        sql<string>`sum(
          case 
            when transactions.transaction_type_id = '1' then transactions.transaction_amount 
            when transactions.transaction_type_id = '5' then transactions.transaction_amount 
            else 0 
          end
        )`.as("expense"),
        sql<string>`budget.budget_amount + sum(
          case 
            when transactions.transaction_type_id = '1' then transactions.transaction_amount
            when transactions.transaction_type_id = '5' then transactions.transaction_amount 
            else 0 
