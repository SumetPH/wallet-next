...
export async function POST(req: NextRequest) {
  try {
    ...
    const createDebtPayment = await db.transaction().execute(async (trx) => {
      const debtPayment = await trx
        .insertInto("debt_payment")
        .values({
    ...
    return Response.json(createDebtPayment);
  } catch (error) {
    console.error(error);
